...
const NAV_ITEMS = [
  ...
  { href: '/activity', label: 'Audit Trail', icon: AuditTrailIcon },
];

const SECONDARY_ITEMS = [
  { href: '#', label: 'My Account', icon: AccountIcon },
];
        ...
        {SECONDARY_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.label}
              href={item.href}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium text-text-muted hover:bg-surface hover:text-text transition-colors"
            >
              <Icon className="w-4 h-4 flex-shrink-0" />
              <span>{item.label}</span>
            </Link>
...
function HelpCenterIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
